    ...
    <form action={action} className="space-y-3" noValidate>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        >
          ...
          {canGrantAdmin && (
            <option value="admin">Admin (this building)</option>
          )}
          <option value="technician">Technician</option>
          <option value="vendor">Vendor</option>
        </Select>
        <Select
          label="Unit (residents)"
          name="unitId"
          required={false}
          defaultValue=""
          error={state.errors?.unitId}
        >
      ...
      <p className="text-xs text-muted">
        They must already have an account (signed up or created in Supabase
        Auth) — this links their existing account to the property.
        {canGrantAdmin
